...
9. **Chunked synthesis and speech-position sync (from the sibling forks)**:
   Long requests are split sentence-by-sentence (`EqText.chunkRanges`, ≤1500 chars per piece) so the 64 KB bridge cap is never hit and stops land between sentences. Opt-in **index marks** (`nativeStreamCreateWithIndices` + `nativeStreamSpeakWithMarks` + `nativeStreamReadIndices`) ride with their audio — the ring is paced to 16 KB of lead while marks are on — and the service turns them into `SynthesisCallback.rangeStart` word-boundary events (the TalkBack highlight). Marks are fed as `eciAddText` segment boundaries + `eciInsertIndex` between segments, so annotations can never straddle a segment and be spoken instead of obeyed. A **hang watchdog** (eloquence-revived's recovery) restarts the process when synthesis stops making progress.
